# IttyBitz Security Audit (historical — superseded)

> **This report predates Keymaker v2 and does not cover it.**
>
> It reviewed IttyBitz v2.2.0, whose only cryptography was PBKDF2-HMAC-SHA-256
> with AES-256-GCM in a headerless container. Keymaker has since added Argon2id,
> ChaCha20-Poly1305, chained AEAD, HKDF subkey derivation, the KEYM container
> parser, authenticated KDF parameters, BIP-39 handling, SeedQR export, and the
> dice entropy tooling. None of that is in scope below.
>
> In particular, its conclusion of "no exploitable vulnerabilities of High or
> Medium severity" applies to the frozen legacy core in `src/lib/crypto.ts`
> only. It must not be read as a statement about KEYM v1.
>
> For the current review, see [SECURITY-AUDIT.md](SECURITY-AUDIT.md).
> Retained unchanged below for the historical record.
