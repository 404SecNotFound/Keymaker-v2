...
- [Encrypted audio carrier](#encrypted-audio-carrier)
...
| Pack an encrypted secret into audio (carrier) | **yes** | no | no | no | no |
...
## Encrypted audio carrier

Under the **Audio** tab. It packs an encrypted secret into an audio file that
plays like ordinary music. The bytes packed into the audio are the same KEYM
container the Encrypt tab writes, so Argon2id, AES-256-GCM and the
[independent decryptor](reference/keym2.py) all still apply. This is a carrier,
not a new cipher. Full layout in
[**docs/FORMAT-AUDIO-STEGO.md**](docs/FORMAT-AUDIO-STEGO.md).

<p align="center">
  <img alt="The Audio tab: a warning that this is an encrypted carrier not a hiding place, a carrier picker, a secret to pack in, and a password" src="docs/screenshots/12-audio-stego.png" width="620" />
</p>

- **Pack:** choose an audio carrier and a secret (text or file), set a password,
  and download a WAV that carries the sealed container in the least-significant
  bit of its samples.
- **Unpack:** open that WAV here with the password to recover the secret.

Two things it states plainly, because a security tool should not oversell hiding:

- **A carrier, not a hiding place.** The password is what protects the secret.
  This is **not** steganography: the `KAUD1` header sits at a fixed place in the
  samples and the payload follows it sequentially, so the presence of a payload
  is trivially detectable. It does not conceal *that* a secret exists.
- **Lossless output only.** Input may be MP3, WAV, FLAC or Ogg, but the file you
  ...
  re-encoding, so there is deliberately no carrier-MP3 out.
...
- **Encrypted audio carrier**: pack the sealed container into an audio file's sample
...
| Header cannot be downgraded | **Cryptographic, with one stated exception.** Every AEAD layer authenticates the header: the payload against the 24-byte core header, each slot's wrap against that plus its own 48-byte prefix. `slot_count` is deliberately in neither, so that a slot table stays editable by someone holding exactly one slot's secret — see [§1.2](docs/FORMAT-V3-DESIGN.md). Everything else is covered, and the slot table is covered separately by `slot_table_mac` — see the next row. |
| The list of unlock methods cannot be edited behind your back | **Detected, not prevented — for v3 containers.** Anyone holding the file can delete a slot; the format cannot stop them, and the loss is silent to everyone except the person whose recovery method vanished. What v3 adds is a `slot_table_mac` keyed from the master key: on unlock the app recomputes it and tells you if the list has changed. It still gives you your data — a warned backup beats a lost one — and it will **not** make any further change to a list that failed the check, because re-signing an altered list would destroy the very evidence it just reported. v1 and v2 containers carry no MAC, so nothing is claimed about theirs. [§5](docs/FORMAT-V3-DESIGN.md) |
