...
try {
  ...
  // ---- 12: the Audio (encrypted carrier) tab, Pack side, with the honest
  // "this is a carrier, not a hiding place" banner and the controls that seal a
  // secret into a carrier. Framed from the heading to the primary button so the
  // band is the whole workflow in one panel. ----
  await page.setViewportSize({ width: 1180, height: 1600 });
  await page.goto(BASE, { waitUntil: 'networkidle' });
  await visible(page.getByRole('tab', { name: 'Audio' })).click();
  await visible(page.getByPlaceholder('Enter the secret text to pack in…')).fill(
    'The safe-deposit key is with Rachel. Recovery codes: 4471-0092, 8823-5510.'
  );
  await settle();
  await shotRegion(
    visible(page.getByRole('heading', { name: 'Encrypted audio carrier' })),
    visible(page.getByRole('button', { name: 'Pack into audio & download WAV' })),
