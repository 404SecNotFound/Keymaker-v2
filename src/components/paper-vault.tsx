    ...
    <div className="paper-vault" aria-hidden="true">
      ...
      {shares && shares.length > 0 ? (
        <section className="pv-block pv-break">
          <h2>Recovery shares — cut apart and separate</h2>
          <p className="pv-note">
            Any <strong>{threshold}</strong> of these {shares.length} open the
            backup <em>without the password</em>. Each one is therefore as
            sensitive as the password itself. Give them to people who would not
            casually combine them, and keep this page no longer than it takes to
            cut it up.
          </p>
          {shares.map((share, i) => (
            <div key={share} className="pv-share">
              <div className="pv-share-head">
                <span>
                  Share {i + 1} of {shares.length}
                </span>
                <span className="pv-holder">Holder: ______________________</span>
              </div>
              <div className="pv-share-body">
                <QRCodeCanvas value={share} size={190} level="M" marginSize={2} />
                <code>{share}</code>
              </div>
              <p className="pv-share-warn">
                Any {threshold} shares open the backup. This one is a key, not a
                receipt.
              </p>
            </div>
          ))}
      ...
      <section className="pv-block pv-break">
        ...
        <ol className="pv-steps">
          ...
          <li>
            ...
            <code>--share</code> once per share if you have {threshold ?? "k"} of
            them.
          </li>
        </ol>
        <p className="pv-note">
          ...
          check the password, the shares, and whether every symbol scanned.
        </p>
      </section>
