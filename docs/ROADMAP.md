...
§4.7 of `FORMAT-V2-DESIGN.md` specifies the slot and is no longer marked design
only: `reference/keym2.py` was written from it, the TypeScript matches byte for
byte under `crosstest2.py`, three fixtures are frozen, and the app enrols and
unlocks with a real (virtual) authenticator under Playwright.

**What the layout decision bought.** Deriving `prf_salt` from the slot salt and
using a discoverable credential together remove every passkey-specific field, so
the slot record's shape does not change for 0x01 any more than it did for 0x02.
The parser is untouched, and the container discloses nothing about the
credential.

**Three things the implementation established that the design did not.**

1. **Enrolment has to ask the authenticator twice.** PRF results at creation
   time are optional, so reading them there would enrol credentials that can
   never unlock anything — a failure the owner meets years later. `create()`
   then `get()`, and only the assertion's output counts.
2. **`residentKey: "required"` is load-bearing**, not a preference. With no
   credential id stored, a non-discoverable credential cannot be found at
   unlock.
3. **Unlock reads before it asks.** The PRF salt derives from the slot salt, so
...
**What the negative controls caught.** Two tests that could not fail. In the
reference, a self-test check comparing the derived PRF salt against a slot key
that differed in IKM as well as info — it could never have failed, and was
replaced with one that catches the plausible "just return the slot salt"
simplification. In the browser suite, "the password still opens a container that
has a passkey" passed on containers with no passkey at all, until it was made to
assert the slot table first. Both were found by disabling the thing under test
